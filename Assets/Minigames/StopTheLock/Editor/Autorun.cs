...
namespace AppAdvisory.StopTheLock
{
	...
	public class Autorun
	{
		...
		static void RunOnce() 
		{
			...
			int count = EditorPrefs.GetInt(WelcomeStopTheLock.PREFSHOWATSTARTUP + "autoshow",0);
			...
			EditorPrefs.SetInt(WelcomeStopTheLock.PREFSHOWATSTARTUP + "autoshow", count + 1);

			WelcomeStopTheLock.showAtStartup = EditorPrefs.GetInt(WelcomeStopTheLock.PREFSHOWATSTARTUP, 1) == 1;
			 
			if (WelcomeStopTheLock.showAtStartup)
			{
				DGChecker.CheckItNow();
			
				WelcomeStopTheLock.OpenPopupStartup();
			}
		}
	}
}         