...
public class UMP_Options : MonoBehaviour
{
    ...
    public void SetController(bool b)
    {
		if (b) {
			currentController = 1;
			ControllerText.text = "No Assist";
		} else {
			currentController = 0;
			ControllerText.text = "Assist";
		}
    }
    ...
    public void Apply()
    {
		...
		PlayerPrefs.SetInt(UMPKeys.Controller, currentController);
    ...
    void GetInfoOptions()
    {
		...
		if (PlayerPrefs.HasKey(UMPKeys.Controller)) { currentController = PlayerPrefs.GetInt(UMPKeys.Controller); }
